...
class TemporalNetwork:
    ...
    @staticmethod
    def readFile(
        filename, sep=",", timestampformat="%Y-%m-%d %H:%M", maxlines=_sys.maxsize
    ):
            ...
            while line and n <= maxlines:
                ...
                try:
                    ...
                    if t >= 0:
                        ...
                        Log.add(
                            "Ignoring negative timestamp in line "
                            + str(n + 1)
                            + ': "'
                            + line.strip()
                            + '"',
                    ...
                    Log.add(
                        "Ignoring malformed data in line "
                        + str(n + 1)
                        + ': "'
                        + line.strip()
                        + '"',
    ...
    def filterEdges(self, edge_filter):
        ...
        Log.add(
            "finished. Filtered out "
            + str(self.ecount() - len(new_t_edges))
            + " time-stamped edges.",
            Severity.INFO,
        )
    ...
    def summary(self):
        ...
        if len(self.ordered_times) > 1:
            summary += (
                "Observation period:\t["
                + str(min(self.ordered_times))
                + ", "
                + str(max(self.ordered_times))
                + "]\n"
            )
            summary += (
                "Observation length:\t"
                + str(max(self.ordered_times) - min(self.ordered_times))
                + "\n"
            )
            ...
            summary += (
                "Min/Max inter-event dt:\t" + str(min(d)) + "/" + str(max(d)) + "\n"
            )
    ...
    def exportUnfoldedNetwork(self, filename):
        ...
        output.append("\\newcounter{a}\n")
        output.append(
            "\\begin{tikzpicture}[->,>=stealth',auto,scale=0.5, every node/.style={scale=0.9}]\n"
        )
        ...
        output.append(
            "\\node[lbl,left=0.5cm of "
            + _np.sort(self.nodes)[0]
