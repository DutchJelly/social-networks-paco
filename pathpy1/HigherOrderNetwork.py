...
class HigherOrderNetwork:
    ...
    def __init__(
        ...
        assert (
            method == "FirstOrderTransitions" or method == "KOrderPi"
        ), "Error: unknown method to build null model"

        assert (
            len(paths.paths.keys()) > 0 and max(paths.paths.keys()) >= k
        ...
        if k > 1:
            ...
            A = g1.getAdjacencyMatrix(
                includeSubPaths=True, weighted=False, transposed=True
            )
        ...
        if not nullModel:
            ...
            self.nodes = list(
                set(self.nodes)
            )  # removes duplicates, does however randomize the order
            ...
            assert (A**k).sum() == len(possiblePaths), (
                "Expected "
                + str((A**k).sum())
                + " paths but got "
                + str(len(possiblePaths))
            )

            if method == "KOrderPi":
                # compute stationary distribution of a random walker in the k-th order network
                g_k = HigherOrderNetwork(
                    paths, k=k, separator=separator, nullModel=False
                )
                pi_k = HigherOrderNetwork.getLeadingEigenvector(
                    g_k.getTransitionMatrix(includeSubPaths=True),
                    normalized=True,
    ...
    def getDoF(self, assumption="paths"):
        ...
        """
        assert (
            assumption == "paths" or assumption == "ngrams"
    ...
    def ClosenessCentrality(self):
        ...
        Log.add(
            "Calculating closeness centralities (k = " + str(self.order) + ") ...",
            Severity.INFO,
        )
    ...
    def EvCent(self, projection="scaled", includeSubPaths=True):
        ...
        A = self.getAdjacencyMatrix(
            includeSubPaths=includeSubPaths, weighted=False, transposed=True
        )
    ...
    def PageRank(
        ...
        assert (
            ...
            or projection == "last"
            or projection == "first"
            or projection == "scaled"
        ), "Invalid projection method"

        Log.add(
            "Calculating PageRank in " + str(self.order) + "-th order network...",
            Severity.INFO,
        )
        ...
        A = self.getAdjacencyMatrix(
            includeSubPaths=includeSubPaths, weighted=False, transposed=False
        )
    ...
    def BetweennessCentrality(self, normalized=False):
        ...
        shortest_paths_firstorder = _co.defaultdict(
            lambda: _co.defaultdict(lambda: set())
        )

        Log.add(
            "Calculating betweenness centralities (k = " + str(self.order) + ") ...",
            Severity.INFO,
        )
        ...
        for sk in shortest_paths:
            for dk in shortest_paths:
                ...
                for pk in shortest_paths[sk][dk]:
                    ...
                    shortest_paths_firstorder[s1][d1].add(
                        self.HigherOrderPathToFirstOrder(pk)
                    )
                    ...
                    for v in p1[1:-1]:
                        if s1 != v != d1:
                            ...
                            node_centralities[v] += 1.0 / (
                                len(shortest_paths_firstorder[s1][d1]) + self.order - 1
                            )
                            # else:
                            #    node_centralities[v] += 1.0
        if normalized:
    ...
    def summary(self):
        ...
        summary += (
            "Total weight (sub/longest):\t"
            + str(self.totalEdgeWeight()[0])
            + "/"
    ...
    def getAdjacencyMatrix(self, includeSubPaths=True, weighted=True, transposed=False):
        ...
        return _sparse.coo_matrix(
            (data, (row, col)), shape=(self.vcount(), self.vcount())
        ).tocsr()
    ...
    def getTransitionMatrix(self, includeSubPaths=True):
        ...
        for s, t in self.edges:
            ...
            if (self.edges[(s, t)][1] > 0) or (
                includeSubPaths and self.edges[(s, t)][0] > 0
            ):
                row.append(self.nodes.index(t))
                col.append(self.nodes.index(s))
                if includeSubPaths:
                ...
                # print(count)
                assert D[self.nodes.index(s)] > 0, (
                    "Encountered zero out-degree for node "
                    + str(s)
                    + " while weight of link ("
                    + str(s)
                    + ", "
                    + str(t)
                    + ") is non-zero."
                ...
                if prob < 0 or prob > 1:
                    tn.Log.add(
                        "Encountered transition probability outside [0,1] range.",
                        Severity.ERROR,
                    )
                    raise ValueError()
                data.append(prob)
