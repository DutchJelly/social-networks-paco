...
class Paths:
    ...
    def fromTemporalNetwork(tempnet, delta=1, maxLength=_sys.maxsize):
        ...
        if maxLength == _sys.maxsize:
            Log.add(
                "Extracting time-respecting paths for delta = " + str(delta) + " ..."
            )
        else:
            Log.add(
                "Extracting time-respecting paths up to length "
                + str(maxLength)
                + " for delta = "
                + str(delta)
                + " ..."
            ...
            for x in self.paths[l]:
                if self.paths[l][x][1] > 0:
                    ...
                    for v in x:
                        newP += (mapping[v],)
                    p.addPathTuple(
                        newP, expandSubPaths=True, frequency=(0, self.paths[l][x][1])
                    )
        return p
    ...
    def getSlowDownFactor(self, k=2, lanczosVecs=15, maxiter=1000):
        ...
        assert (
            k > 1
        ), "Slow-down factor can only be calculated for orders larger than one"
    ...
    def getEntropyGrowthRateRatio(
        self, method="MLE", k=2, lanczosVecs=15, maxiter=1000
    ):
        """
        ...
        Tk = gk.getTransitionMatrix()
        Tk_pi = HigherOrderNetwork.getLeadingEigenvector(
            Tk, normalized=True, lanczosVecs=lanczosVecs, maxiter=maxiter
        )
    ...
    def getDistanceMatrix(self):
        ...
        """
        shortest_path_lengths = _co.defaultdict(
            lambda: _co.defaultdict(lambda: _np.inf)
        )

        Log.add(
            "Calculating distance matrix based on empirical paths ...", Severity.INFO
        )
    ...
    def getShortestPaths(self):
        ...
        Log.add(
            "Calculating shortest paths based on empirical paths ...", Severity.INFO
        )
