...
from pathpy2 import Network, Paths, HigherOrderNetwork, MultiOrderModel
from pathpy2.utils import Log, Severity
from pathpy2.utils import PathpyError, PathpyNotImplemented
...
def modular_description_length(network, module_map):
    ...
    assert isinstance(
        network, Network
    ), "argument must be an instance of pathpy.Network"
    assert (
        not network.directed
    ), "this function is currently only implemented for undirected and unweighted networks"
    ...
    for m in module_to_nodes:
        ...
        for n in module_to_nodes[m]:
            x = (node_count[n] + exit_count_node[n]) / (
                2 * within_count[m] + exit_count[m]
            )
            H_Pi += x * _np.log2(x)
...
def find_communities_sa(obj, iterations=100, initial_map=None, T=0.1, t_i=1):
    ...
    """
    assert isinstance(obj, Paths) or isinstance(
        obj, Network
    ), "Can only find communities based on paths or undirected network"
    ...
    with warnings.catch_warnings():
        ...
        for i in range(iterations):
            ...
            if mdl_new != _np.nan and (
                mdl_new <= mdl or _np.random.ranf() < _np.exp(-(mdl_new - mdl) / T)
            ):
                mdl = mdl_new
            else:  # revert change
