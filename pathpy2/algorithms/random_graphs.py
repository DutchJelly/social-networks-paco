...
from pathpy2.classes import Network
from pathpy2.classes import TemporalNetwork
...
def is_graphic_sequence(degree_sequence, self_loops=False, multi_edges=False):
    ...
    """
    assert not (
        not self_loops and multi_edges
    ), "Networks with multi_edges and no self_loops are not supported"
...
def molloy_reed(degree_sequence, node_names=None, self_loops=True, multi_edges=False):
    ...
    assert is_graphic_sequence(
        degree_sequence, self_loops=self_loops, multi_edges=multi_edges
    ), "Error: degree sequence is not graphic"
    ...
    assert (
        len(node_names) >= n
    ), "Error: Number of node names not matching degree sequence length"
...
def erdoes_renyi_gnm(
    n, m, node_names=None, self_loops=True, directed=False, temporal=False
):
    """ """
    if node_names is None:
        node_names = [str(x) for x in range(n)]
...
def erdoes_renyi_gnp(
    n, p, node_names=None, self_loops=True, directed=False, temporal=False
):
    """ """
    if node_names is None:
        node_names = [str(x) for x in range(n)]
            ...
            if i != j or self_loops:
                if _np.random.rand() <= p:
                    if not temporal:
                        ...
                        time += 1
                        network.add_edge(
                            node_names[i], node_names[j], time, directed=directed
                        )
    return network
        ...
        for j in range(n_init):
            if i < j:
                if not temporal:
                    ...
                    network.add_edge(
                        str(node_names[i]), str(node_names[j]), time, directed=directed
                    )
