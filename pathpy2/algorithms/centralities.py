...
from pathpy2 import HigherOrderNetwork
from pathpy2 import Network
from pathpy2 import Paths
from pathpy2.algorithms.shortest_paths import *

from pathpy2.utils import PathpyNotImplemented
...
def degree(network, mode="degree"):
    ...
    for n in network.nodes:
        ...
        if not network.directed and (
            mode == "degree" or mode == "outdegree" or mode == "indegree"
        ):
            degree_centralities[n] = float(network.nodes[n]["degree"])
        if not network.directed and (
            mode == "weight" or mode == "outweight" or mode == "inweight"
        ):
...
def _bw(higher_order_net, normalized=False):
    ...
    """
    assert isinstance(
        higher_order_net, HigherOrderNetwork
    ), "arguments must be an instance of HigherOrderNetwork"

    Log.add(
        "Calculating betweenness (order k = %s) ..." % higher_order_net.order,
        Severity.INFO,
    )
...
def _cl(higher_order_net, normalized=False):
    ...
    Log.add(
        "Calculating closeness (k = %s) ..." % higher_order_net.order, Severity.INFO
    )
...
def _ev(network, projection="scaled", include_sub_paths=True):
    ...
    adj_mat = network.adjacency_matrix(
        include_subpaths=include_sub_paths, weighted=False, transposed=True
    ).asfptype()
    # calculate leading eigenvector of A
    _, v = sla.eigs(adj_mat, k=1, which="LM")
...
def pagerank(network, alpha=0.85, max_iter=100, tol=1.0e-6, weighted=False):
    ...
    q_mat = sparse.spdiags(
        row_sums.T, 0, adj_mat.shape[0], adj_mat.shape[1], format="csr"
    )
    ...
    for _ in range(max_iter):
        ...
        p_rank = (
            alpha * (p_rank * q_mat + sum(p_rank[d]) * inv_n_nodes)
            + (1 - alpha) * inv_n_nodes
        )
...
def _pr_ho(
    ...
    # entries A[s,t] give directed link s -> t
    adj_mat = network.adjacency_matrix(
        include_subpaths=include_sub_paths, weighted=weighted, transposed=False
    ).asfptype()
