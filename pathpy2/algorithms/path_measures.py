...
from pathpy2.utils import PathpyError


__all__ = [
    "slow_down_factor",
    "entropy_growth_rate_ratio",
    "betweenness_preference",
    "betweenness_preference_matrix",
]
...
def entropy_growth_rate_ratio(
    paths, method="MLE", k=2, lanczos_vectors=15, maxiter=1000
):
    ...
    if method == "Miller":
        ...
        print("N = ", num_obs)
        h_mat_k = -np.sum(tran_mat_k * leading_eigen_vec_k) + (possible_k_paths - 1) / (
            2 * num_obs
        )
    else:
