...
from pathpy2.algorithms.shortest_paths import shortest_paths
    ...
    with open(filename, "r") as f:
        ...
        while line:
            ...
            origin_destination_list.append(
                (fields[0].strip(), fields[1].strip(), float(fields[2].strip()))
            )
            line = f.readline()
    Log.add("Finished.")
...
def paths_from_origin_destination(
    origin_destination_list, network, distribute_weight=True
):
    ...
    for o, d, w in origin_destination_list:
        assert o in network.nodes, (
            "Error: could not find node " + str(o) + " in network"
        )
        assert d in network.nodes, (
            "Error: could not find node " + str(d) + " in network"
        )
