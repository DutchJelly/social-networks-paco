    ...
    for u in network.predecessors[focal_node]:
        for w in network.successors[focal_node]:
            if self_loops or (u != focal_node and w != focal_node):
                ...
                w_2 = w_1 * (
                    _np.sum(network.edges[(focal_node, w)]["weight"]) / out_weight
                )
    ...
    if markov:
        ...
        for p in paths.paths[steps]:
            if p[0] == node:
                for t in range(len(p) - 1):
                    flow_net.add_edge(
                        "{0}_{1}".format(p[t], t),
                        "{0}_{1}".format(p[t + 1], t + 1),
                        weight=paths.paths[steps][p].sum(),
                    )
        ...
        while Q:
            ...
            for w in flow_net.successors[v]:
                flow_net.nodes[w]["inweight"] = (
                    flow_net.nodes[w]["inweight"] - flow_net.edges[(v, w)]["weight"]
                )
                flow_net.edges[(v, w)]["weight"] = (
                    inweight / outweight
                ) * flow_net.edges[(v, w)]["weight"]
                flow_net.nodes[w]["inweight"] = (
                    flow_net.nodes[w]["inweight"] + flow_net.edges[(v, w)]["weight"]
