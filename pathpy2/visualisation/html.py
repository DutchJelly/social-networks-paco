...
from pathpy2.classes.network import Network
from pathpy2.classes.higher_order_network import HigherOrderNetwork
from pathpy2.classes.multi_order_model import MultiOrderModel
from pathpy2.classes.temporal_network import TemporalNetwork
from pathpy2.classes.paths import Paths

from pathpy2.visualisation.alluvial import generate_memory_net
from pathpy2.visualisation.alluvial import generate_diffusion_net
from pathpy2.visualisation.alluvial import generate_memory_net_markov
...
def plot(network, **params):
    ...
    """
    assert isinstance(network, Network) or isinstance(
        network, MultiOrderModel
...
def generate_html(network, **params):
    ...
    def get_attr(key, attr_name, attr_default):
        ...
        # raise exception if parameter is of unexpected type
            ...
            raise Exception(
                "Edge and node attribute must either be dict or {0}".format(
                    type(attr_default)
                )
            )
    ...
    def compute_weight(network, e):
        ...
        if "force_weighted" in params and not params["force_weighted"]:
            ...
            source_weight = (
                network.nodes[e[0]]["indegree"] + network.nodes[e[0]]["outdegree"]
            )
            target_weight = (
                network.nodes[e[1]]["indegree"] + network.nodes[e[1]]["outdegree"]
            )
        else:
            weight = network.edges[e]["weight"]
            source_weight = (
                network.nodes[e[0]]["inweight"] + network.nodes[e[0]]["outweight"]
            )
            target_weight = (
                network.nodes[e[1]]["inweight"] + network.nodes[e[1]]["outweight"]
            )
        ...
        "links": [
            {
                ...
                "weight": (
                    compute_weight(network, e) if hon is None and mog is None else 0.0
                ),
            }
    ...
    if hon is not None:
        for e in hon.edges:
            ...
            if (v, w) in higher_order_forces.edges:
                weight = (
                    higher_order_forces.edges[(v, w)]["weight"] + hon.edges[e]["weight"]
                )
            else:
    ...
    if mog is not None:
        for l in range(1, mog.max_order + 1):
            for e in mog.layers[l].edges:
                ...
                if (v, w) in higher_order_forces.edges:
                    weight = (
                        higher_order_forces.edges[(v, w)]["weight"]
                        + mog.layers[l].edges[e]["weight"]
                    )
                else:
...
def _generate_html_tempnet(tempnet, **params):
    ...
    network_data = {
        ...
        "links": [
            {
                "source": fix_node_name(s),
                "target": fix_node_name(v),
                "width": 1,
                "time": t,
            }
            for s, v, t in tempnet.tedges
        ],
    }
    ...
    d3js_params = {"network_data": json.dumps(network_data), "div_id": div_id}
...
def _generate_html_paths(paths, **params):
    ...
    if "markov" in params and params["markov"]:
        n = generate_memory_net_markov(
            HigherOrderNetwork(paths, k=1), node, self_loops=self_loops
        )
    else:
...
def generate_html_diffusion(paths, **params):
    ...
    d3js_params = {"flow_data": json.dumps(data), "div_id": div_id}
