...
@singledispatch
def export_tikz(
    tempnet, filename, dag=True, angle=20, layer_dist="0.3cm", split_directions=True
):
    ...
    for n in _np.sort(tempnet.nodes):
        if last == "":
            ...
            node_fmt = (
                r"\node[lbl, right=0.4cm of {last}-0] ({n}-0) " r"{{\bf \Huge {n} }};"
            )
        ...
        for edge in tempnet.time[ts]:
            if dag:
                ...
                if (edge[1], edge[0], ts) not in tempnet.time[ts]:
                    ...
                    edge_str = edge_fmt.format(
                        edge[0], ts, bend_direction, angle, edge[1], ts
                    )
                    output.append(edge_str)
    output.append(";\n")
