...
class TemporalNetwork:
    ...
    @classmethod
    def from_sqlite(
        cls, cursor, directed=True, timestamp_format="%Y-%m-%d %H:%M:%S", time_rescale=1
    ):
        ...
        for row in cursor:
            # r = sqlite3.Row(row)
            timestamp = row["time"]
            assert isinstance(timestamp, int) or isinstance(
                timestamp, str
            ...
            tedges.append(
                (str(row["source"]), str(row["target"]), int(t / time_rescale))
            )
            if not directed:
                tedges.append(
                    (str(row["target"]), str(row["source"]), int(t / time_rescale))
                )
    ...
    def read_file(
        ...
        with open(filename, "r") as f:
            ...
            if time_ix < 0:  # pragma: no cover
                Log.add(
                    "No time stamps found in data, assuming consecutive links",
                    Severity.WARNING,
                )
                ...
                try:
                    ...
                    if t >= 0 and fields[source_ix] != "" and fields[target_ix] != "":
                        ...
                        if fields[source_ix] == "" or fields[target_ix] == "":
                            ...
                            msg = "Negative timestamp in line {0}: {1}".format(
                                n + 1, s_line
                            )
    ...
    def write_file(self, filename, separator=","):
        ...
        """
        msg = "Writing {0} time-stamped edges to file {1}".format(
            self.ecount(), filename
        )
    ...
    def add_edge(
        self, source, target, ts, directed=True, timestamp_format="%Y-%m-%d %H:%M:%S"
    ):
        ...
        """
        assert isinstance(ts, int) or isinstance(
            ts, str
    ...
    def shuffle_edges(
        self, l=0, with_replacement=False, window_splits=None, maintain_undirected=True
    ):
            ...
            if with_replacement:  # sample l edges with replacement
                ...
                while i < l:
                    ...
                    if maintain_undirected and (edge[1], edge[0], edge[2]) in edges:
                        ...
                        if (edge[0], edge[1], time) not in tedges and (
                            edge[1],
                            edge[0],
                            time,
                        ) not in tedges:
