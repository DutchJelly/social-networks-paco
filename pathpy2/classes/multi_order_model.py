...
from pathpy2.utils import Log, Severity
from pathpy2.utils.exceptions import PathpyError, PathsTooShort, PathpyNotImplemented
from pathpy2.classes.higher_order_network import HigherOrderNetwork
from pathpy2.classes.paths import Paths
...
class MultiOrderModel:
    ...
    def __add_layers_parallel(self, orders):
        ...
        try:
            ...
            raise ImportError(
                'Error while importing module "pathos",'
    ...
    def add_layers(self, max_order):
        ...
        """
        from pathpy2 import ENABLE_MULTICORE_SUPPORT
    ...
    def summary(self):
        ...
        layer_fmt = (
            "Layer k = {k} \t {ncount} nodes, {ecount} links, {sum_path} paths, "
    ...
    def likelihood(self, paths=None, max_order=None, log=True):
        ...
        for k in range(0, max_order + 1):
            if k < max_order:
                p = self.layer_likelihood(
                    paths, k, consider_longer_paths=False, log=True
                )[0]
            else:
                p = self.layer_likelihood(
                    paths, k, consider_longer_paths=True, log=True
                )[0]
            ...
            for p in paths.paths[k]:
                ...
                if freq > 0:
                    ...
                    likelihood += self.path_likelihood(
                        p, freq, l, log=True, index_maps=indexmaps
                    )
            if n == 0:
                likelihood = 0
        if log:
...
observed and therefore the likelihood cannot be computed.
        ...
        if index_maps is None:
            index_maps = {
                k: self.layers[k].node_to_name_map() for k in range(0, layer + 1)
            }
        ...
        if layer == 0:
            try:
                for s in range(len(path)):
                    ...
                    likelihood += (
                        np.log(self.transition_matrices[0][target, source]) * freq
                    )
            except KeyError as e:
                msg = (
                    "The path segment '({})' has not been observed and therefore the "
    ...
    def likelihood_ratio_test(
        ...
        x = -2 * (
            self.likelihood(paths, max_order=max_order_null, log=True)
        ...
        dof_diff = self.degrees_of_freedom(
            max_order=max_order, assumption=assumption
        ) - self.degrees_of_freedom(max_order=max_order_null, assumption=assumption)

        Log.add(
            "Likelihood ratio test for K_opt = " + str(max_order) + ", x = " + str(x)
        )
    ...
    def estimate_order(
        self, paths=None, stop_at_order=None, significance_threshold=0.01
    ):
        ...
        for k in range(2, stop_at_order + 1):
            ...
            if k >= self.max_order:
                try:
                    ...
                    msg = (
                        "Optimal order is at least %d, but could be higher. Paths too short"
        ...
        if stop_at_order == max_accepted_order and max_len > stop_at_order:
            msg = (
                "Optimal order is at least %d, but may be higher."
    ...
    def test_network_hypothesis(self, paths, method="AIC"):
        ...
        """
        from pathpy2.utils.exceptions import PathpyError
        ...
        likelihood_0, n_0 = self.layer_likelihood(
            paths, l=0, consider_longer_paths=True, log=True, min_path_length=1
        )

        # log-likelihood and observation count of first-order model
        likelihood_1, n_1 = self.layer_likelihood(
            paths, l=1, consider_longer_paths=True, log=True, min_path_length=1
        )
        ...
        if method == "AIC":
            ...
            ic0 = (
                2 * dof0
                - 2 * likelihood_0
                + (2 * (dof0 + 1) * (dof0 + 2)) / (n_0 - dof0 - 2)
            )
            ic1 = (
                2 * dof10
                - 2 * likelihood_1
                + (2 * (dof10 + 1) * (dof10 + 2)) / (n_1 - dof10 - 2)
            )
        elif method == "BIC":
            ...
            raise PathpyError(
                "Method check has not filtered out illegal " "method %s " % method
            )
