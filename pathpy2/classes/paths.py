...
from pathpy2.utils.default_containers import nested_zero_default as _nested_zero_default
from pathpy2.utils.default_containers import zero_array_default as _zero_array_default
...
class Paths:
    ...
    @staticmethod
    def read_edges(
        filename, separator=",", weight=False, undirected=False, maxlines=None
    ):
        """
            ...
            while line and n <= maxlines:
                ...
                if frequency:
                    ...
                    if freq > 0:
                        ...
                        Log.add(
                            "Non-positive path count in line {0}".format(n),
                            Severity.WARNING,
                        )
                else:
                    for field in fields:
    ...
    def add_path(self, path, frequency=1, expand_subpaths=True, separator=","):
        ...
        """
        assert (
            isinstance(path, tuple) or isinstance(path, list) or isinstance(path, str)
        ), "Path must be tuple or ngram string."
        ...
        for x in path:
            if isinstance(x, str) and self.separator in x:
                raise PathpyError(
                    "Node name contains separator character. "
    ...
    def filter_nodes(
        self, node_filter, min_length=0, max_length=sys.maxsize, split_paths=True
    ):
