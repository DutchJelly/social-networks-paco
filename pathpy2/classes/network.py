...
from pathpy2.utils.exceptions import PathpyError, PathpyNotImplemented
...
class Network:
    ...
    @classmethod
    def read_file(
        cls, filename, separator=",", weighted=False, directed=False, header=False
    ):
        ...
        with open(filename, "r") as f:
            ...
            for n, line in enumerate(f):
                ...
                if len(fields) < 2:
                    Log.add(
                        "Ignoring malformed line {0}: {1}".format(
                            n, line + header_offset
                        ),
                        Severity.WARNING,
                    )
                else:
    ...
    def write_file(self, filename, separator=",", weighted=False, header=False):
        ...
        with open(filename, "w+") as f:
            if header:
                if weighted:
                    f.write(
                        "source" + separator + "target" + separator + "weight" + "\n"
                    )
                else:
    ...
    def from_sqlite(cls, cursor, directed=True):
        ...
        """
        from pathpy2.classes import DAG
    ...
    def from_temporal_network(
        cls, tempnet, min_time=None, max_time=None, directed=True
    ):
        ...
        for v, w, t in tempnet.tedges:
            if (min_time is None or t >= min_time) and (
                max_time is None or t < max_time
            ):
    ...
    def remove_edge(self, source, target):
        ...
        if self.directed:
            ...
            if source != target:
                ...
                self.nodes[target]["outweight"] -= self.edges[(source, target)][
                    "weight"
                ]
                self.nodes[target]["inweight"] -= self.edges[(source, target)]["weight"]
    ...
    def find_edges(self, select_nodes=lambda v, w: True, select_edges=lambda e: True):
        ...
        return [
            ...
            for e in self.edges
            if (
                select_nodes(self.nodes[e[0]], self.nodes[e[1]])
                and select_edges(self.edges[e])
            )
        ]
    ...
    def degrees(self, mode="degree"):
        ...
        """
        assert (
            mode is "degree" or mode is "indegree" or mode is "outdegree"
        ...
        for s, t in self.edges:
            ...
            if weight > 0:
                ...
                if prob < 0 or prob > 1:  # pragma: no cover
                    raise ValueError(
                        "Encountered transition probability {p} outside "
    ...
    def laplacian_matrix(self, weighted=False, transposed=False):
        ...
        if weighted:
            ...
            D = _sparse.diags(
                _np.array([float(self.nodes[v]["degree"]) for v in self.nodes])
            )
        L = D - A
        if transposed:
            return L.transpose()
    ...
    def summary(self):
        ...
        summary = summary_fmt.format(
            directed_str=directed_str, ncount=self.ncount(), ecount=self.ecount()
        )
        return summary
    ...
    def _repr_html_(self):
        ...
        """
        from pathpy2.visualisation.html import generate_html
...
def network_from_networkx(graph):
    ...
    if isinstance(graph, nx.DiGraph):
        ...
        raise PathpyNotImplemented(
            "At the moment only DiGraph and Graph are supported."
        )
...
def network_to_networkx(network):
    ...
    for node_id, node_props in network.nodes.items():
        valid_props = {
            k: v for k, v in node_props.items() if k not in excluded_node_props
        }
