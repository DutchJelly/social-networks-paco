...
class DAG(Network):
    ...
    def add_edges(self, edges):
        ...
        if redundant_edges > 0:
            Log.add(
                "Warning: omitted %d redundant edges" % redundant_edges,
                Severity.WARNING,
            )
    ...
    def make_acyclic(self):
        ...
        if not self.is_acyclic:
            ...
            assert (
                self.is_acyclic
            ), "Error: make_acyclic did not generate acyclic graph!"
            Log.add(
                "Removed " + str(removed_links) + " back links to make graph acyclic",
                Severity.INFO,
            )
    ...
    @classmethod
    def read_file(
        cls, filename, separator=",", maxlines=None, mapping=None, header=False
    ):
        """
            ...
            for i, line in enumerate(f):
                ...
                try:
                    if mapping is None or (
                        fields[0] in mapping and fields[1] in mapping
                    ):
                        edges.append((fields[0], fields[1]))

                except (IndexError, ValueError):  # pragma: no cover
                    msg = "Ignoring malformed data in " 'line {}: "{}"'.format(
                        (i + header), line.strip()
                    )
