...
from pathpy2.utils.exceptions import PathsTooShort
...
class HigherOrderNetwork(Network):
    ...
    def __init__(self, paths, k=1, null_model=False, separator=None):
        ...
        if not (paths.paths.keys() and max(paths.paths.keys()) >= k):
            msg = (
                "Constructing a model of order %d requires paths of at least length %d, "
        ...
        if k > 1:
            ...
            A = g1.adjacency_matrix(
                include_subpaths=True, weighted=False, transposed=True
            )
        ...
        if not null_model:
            ...
            if k == 0:
                ...
                self.add_node(
                    "start",
                    inweight=_np.array([0.0, 0.0]),
                    outweight=_np.array([0.0, 0.0]),
                )
            ...
            assert A_sum == len(
                possible_paths
            ), "Expected {ak} paths but got {re}".format(
                ak=A_sum, re=len(possible_paths)
            )
            ...
            for p in possible_paths:
                ...
                # create nodes and make sure that in- and out-weights are numpy arrays
                self.add_node(
                    v, inweight=_np.array([0.0, 0.0]), outweight=_np.array([0.0, 0.0])
                )
                self.add_node(
                    w, inweight=_np.array([0.0, 0.0]), outweight=_np.array([0.0, 0.0])
                )
        ...
        if k == 0:
            ...
            if null_model or k == 1:
                ...
                null_model_hon = HigherOrderNetwork(
                    paths, k=k, null_model=True, separator=separator
                )
                non_zero = 0
    ...
    def summary(self):
        ...
        summary = summary_fmt.format(
            ...
            ncount=self.ncount(),
            ecount=self.ecount(),
            sub_w=self.total_edge_weight()[0],
            uni_w=self.total_edge_weight()[1],
        )
        return summary
    ...
    def adjacency_matrix(self, include_subpaths=True, weighted=True, transposed=False):
        ...
        if not weighted:
            ...
            if include_subpaths:
                data = _np.array(
                    [float(self.edges[e]["weight"].sum()) for e in self.edges]
                )
            else:
                data = _np.array(
                    [float(self.edges[e]["weight"][1]) for e in self.edges]
                )
