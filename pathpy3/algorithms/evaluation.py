...
def train_test_split(
    ...
    if split == "node":
        ...
        train_nodes = [
            v.uid for v in network.nodes if v.uid not in test_network.nodes.uids
        ]
        for v in train_nodes:
            train_network.add_node(network.nodes[v])
        for e in network.edges:
            if (
                e.v.uid in test_network.nodes.uids
                and e.w.uid in test_network.nodes.uids
            ):
                test_network.add_edge(e)
            if (
                e.v.uid in train_network.nodes.uids
                and e.w.uid in train_network.nodes.uids
            ):
        ...
        train_edges = [
            e.uid for e in network.edges if e.uid not in test_network.edges.uids
        ]
        for e in train_edges:
            train_network.add_edge(network.edges[e])
    else:
        raise NotImplementedError(
            'Unsupported split method "{0}" for instance of type Network'.format(split)
        )
...
def _(
    ...
    """
    test_network = TemporalNetwork(
        directed=network.directed,
        multiedges=network.multiedges,
        uid=network.uid + "_test",
    )
    train_network = TemporalNetwork(
        directed=network.directed,
        multiedges=network.multiedges,
        uid=network.uid + "_train",
    )
    ...
    if split == "time":
            ...
            if i <= split_point:
                if e in train_network.edges:
                    ...
                    train_network.add_edge(
                        e.v, e.w, start=e.start, end=e.end, uid=e.uid
                    )
            else:
                if e in test_network.edges:
                    test_network.add_edge(e, start=e.start, end=e.end)
        ...
        raise NotImplementedError(
            'Unsupported split method "{0}" for instance of type TemporalNetwork'.format(
                split
            )
        )
...
def shuffle_temporal_network(net: TemporalNetwork):
    ...
    shuffled_net = TemporalNetwork(
        directed=net.directed,
        multiedges=net.multiedges,
        uid="{0}_shuffled".format(net.uid),
        **net.attributes,
    )
