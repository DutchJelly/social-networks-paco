...
from pathpy3.models import network as net
...
@singledispatch
def distance_matrix(
    self, weight: Optional[str] = None, count: bool = False
) -> np.ndarray:
...
def _dm_network(
    network: BaseNetwork, weight: Optional[str] = None, count: bool = False
) -> np.ndarray:
    """Calculates shortest path distances between all pairs of nodes
...
def _dm_paths(
    paths: PathCollection, weight: Optional[str] = None, count: bool = False
) -> np.ndarray:
    """ """
...
def all_shortest_paths(
    ...
    for k in tqdm(
        network.nodes.keys(), desc="calculating shortest paths between all nodes"
    ):
    ...
    if return_distance_matrix:
        ...
        for v in network.nodes:
            for w in network.nodes:
                dist_arr[network.nodes.index[v.uid], network.nodes.index[w.uid]] = dist[
                    v.uid
                ][w.uid]
        return s_p, dist_arr
    else:
...
def single_source_shortest_paths(
    network: Network, source: str, weight: Union[bool, str, None] = None
) -> Union[dict, np.array]:
...
def diameter(network: Network, weight: Union[str, bool, None] = None) -> float:
...
def all_longest_paths(
    network: Network, weight: Union[str, bool, None] = None
) -> defaultdict:
...
def avg_path_length(
    network: Network, weight: Union[str, bool, None] = None, exclude_zero: bool = True
) -> float:
