...
# Time-stamp: <Tue 2021-08-24 18:06 juergen>
#
...
from pathpy3.models.higher_order_network import (
    HigherOrderEdge,
    HigherOrderNetwork,
    HigherOrderNode,
)

from pathpy3.core.node import NodeCollection
...
from pathpy3.models.temporal_network import TemporalNetwork, TemporalNode
from pathpy3.models.network import Node
from pathpy3.models.network import Edge

from pathpy3.algorithms.matrices import adjacency_matrix
...
class RandomWalk(BaseProcess):
    ...
    def __init__(
        self, network: Network, weight: Optional[Weight] = None, restart_prob: float = 0
    ) -> None:
        ...
        self.samplers = {
            v: VoseAliasSampling(
                np.nan_to_num(
                    np.ravel(
                        self._transition_matrix[network.nodes.index[v], :].todense()
                    )
                )
            )
    ...
    def node_state(self, v: str) -> bool:
        ...
        if v in self._network.nodes:
            ...
            raise NotImplementedError(
                "Random walk not implemented for network of type {0}".format(
                    type(self._network)
                )
            )
    ...
    def compute_transition_matrix(
        network: Network, weight: Optional[Weight] = None, restart_prob: float = 0
        ...
        for i in range(n):
            ...
            for j in range(n):
                if D[i] > 0:
                    T[i, j] = (
                        restart_prob * (1.0 / n) + (1 - restart_prob) * A[i, j] / D[i]
                    )
                else:
                    if restart_prob > 0:
                        T[i, j] = 1.0 / n
    ...
    def transition_probabilities(self, node: str) -> np.array:
        ...
        return np.nan_to_num(
            np.ravel(
                self._transition_matrix[self._network.nodes.index[node], :].todense()
            )
        )
    ...
    def transition_matrix_pd(self) -> DataFrame:
        ...
        return DataFrame(
            ...
            columns=[v for v in self._network.nodes.index],
            index=[v for v in self._network.nodes.index],
        )
    ...
    def get_path(
        ...
        walk_steps = list(
            data.loc[(data["run_id"] == run_id) & (data["state"] == True)][
                "node"
            ].values
        )
        ...
        # generate Path
        return Path(
            *[walk_steps[i] for i in range(len(walk_steps))],
            directed=True,
            ordered=True,
        )

    def get_paths(
        self, data: DataFrame, run_ids: Optional[Iterable] = None
    ) -> PathCollection:
...
class HigherOrderRandomWalk(RandomWalk):
    ...
    def init(self, seed) -> None:
        ...
        self._first_order_visitations[
            self._first_order_network.nodes.index[
                self._network.nodes[seed].relations[-1]
            ]
        ] = 1
    ...
    def first_order_stationary_state(self, **kwargs) -> np.array:
        ...
        for v in self._network.nodes:
            ...
            first_order_stationary_state[
                self._first_order_network.nodes.index[v1]
            ] += higher_order_stationary_dist[self._network.nodes.index[v.uid]]
    ...
    def first_order_total_variation_distance(self) -> float:
        ...
        """
        return self.TVD(
            self.first_order_stationary_state(), self.first_order_visitation_frequencies
        )
    ...
    def step(self) -> Iterable[str]:
        ...
        self._first_order_visitations[
            self._first_order_network.nodes.index[
                self._network.nodes[current_node].relations[-1]
            ]
        ] += 1
    ...
    def plot(
        ...
        **kwargs
        ...
        for index, row in evolution.iterrows():
            ...
            tn.nodes[first_order_node][row["time"] * timescale, "color"] = (
                self.state_to_color(row["state"])
            )
