...
# Time-stamp: <Tue 2021-08-24 18:04 juergen>
#
...
from pathpy3.core.classes import BaseClass
...
from pathpy3 import tqdm, logger
...
class BaseProcess:
    ...
    def run_experiment(
        self, steps: int, runs: Optional[Union[int, Iterable[Any]]] = 1
    ) -> DataFrame:
    ...
    def plot(
        self, data: DataFrame, run_id: int = 0, timescale: Optional[int] = 1, **kwargs
    ):
        """
        ...
        **kwargs
        ...
        for v in self.network.nodes.uids:
            tv = TemporalNode(v)
            tn.add_node(tv)
            tv[start_time, "color"] = self.state_to_color(
                evolution.loc[
                    (evolution["time"] == start_time) & (evolution["node"] == v)
                ]["state"].values[0]
            )
        ...
        if isinstance(self.network, TemporalNetwork):
            for edge in self.network.edges[start_time:end_time]:
                tn.add_edge(
                    edge.v.uid,
                    edge.w.uid,
                    start=max(edge.start, start_time),
                    end=min(end_time, edge.end),
                )

            # update states
            for index, row in evolution.iterrows():
                tn.nodes[row["node"]][row["time"], "color"] = self.state_to_color(
                    row["state"]
                )
        # if process is simulated on static network
            ...
            for e in self.network.edges:
                tn.add_edge(
                    e.v.uid, e.w.uid, start=start_time, end=end_time * timescale
                )

            # update states
            for index, row in evolution.iterrows():
                tn.nodes[row["node"]][row["time"] * timescale, "color"] = (
                    self.state_to_color(row["state"])
                )
        return tn.plot(node_color=self.state_to_color(False), **kwargs)

    def to_directed_acylic_graph(
        self,
        data: DataFrame,
        run_id: Optional[int] = 0,
        ...
        for index, row in run.iterrows():
            ...
            for v in self._network.predecessors[w]:
                ...
                if len(candidates) > 0:
                    ...
                    if last_state in states and (
                        time_delta is None or (t - last_time) < time_delta
                    ):
