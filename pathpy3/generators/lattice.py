...
def lattice_network(
    start: Optional[int] = 0, stop: Optional[int] = 10, dims: Optional[int] = 2
) -> Network:
    """
    ...
    for pos in _multi_dim_range(start, stop, dims):
        network.add_node(
            Node("".join(str(i) + "-" for i in pos).strip("-"), pos=np.array(pos))
        )

    for v in network.nodes:
        for w in network.nodes:
            if (
                np.sum(np.abs(v["pos"] - w["pos"])) == 1
                and (v.uid, w.uid) not in network.edges
            ):
