...
from pathpy3.core.api import Node, Edge
...
def max_edges(
    n: int, directed: bool = False, multiedges: bool = False, loops: bool = False
) -> Union[int, float]:
...
def ER_nm_randomize(
    network: Network, loops: bool = False, multiedges: bool = False
) -> Union[Network, None]:
...
def generate_degree_sequence(
    n,
    distribution: Union[
        Dict[float, float], scipy.stats.rv_continuous, scipy.stats.rv_discrete
    ],
    **distribution_args,
) -> np.array:
    ...
    # use scipy rv objects to generate graphic degree sequence
        ...
        s = [1]
        LOG.error(
            "Distribution must be either dict, scipy.stats.rv_continuous or scipy.stats.rv_discrete"
        )
        return s


def Molloy_Reed(
    degrees: Union[np.array, Dict[int, float]],
...
def k_regular_random(
    k: int, n: Optional[int] = None, node_uids: Optional[list] = None
) -> Optional[Network]:
    ...
    if n is None and node_uids is None:
        msg = (
            "You must either pass a list of node uids or a number of nodes to generate"
        )
        LOG.error(msg)
        raise ParameterError(msg)
