...
class MultiOrderMatrix:
    def __init__(self, matrix, node_id_dict):
        ...
        self.node_id_dict = {
            (k,) if not type(k) == tuple else k: v for k, v in node_id_dict.items()
        }
        self.id_node_dict = {v: k for k, v in self.node_id_dict.items()}
        self.nodes = list(
            set(
                (fon,)
                for hon in self.node_id_dict.keys()
                for fon in hon
                if not fon == "+"
            )
        )
        self.nodes += list(
            set(hon[-2:] for hon in self.node_id_dict.keys() if hon[-1] == "+")
        )
    ...
    def integrate_zero_order(self):
        ...
        idx = list(
            v for k, v in self.node_id_dict.items() if not k in start_nodes + end_nodes
        )
    ...
    def to_first_order(self):
        ...
        hon = np.matrix(
            [
                fon_id_dict[
                    (
                        (self.id_node_dict[i][-1],)
                        if self.id_node_dict[i][-1] != "+"
                        else self.id_node_dict[i][-2:]
                    )
                ]
...
class MOGen:
    ...
    def __init__(self, paths, max_order=1, model_selection=True):
        """Initialise MOGen."""
        self.paths = {
            tuple(n for n in paths[k].nodes): v for k, v in paths.counter.items()
        }
    ...
    def _chunks(self, iterable, n):
        ...
        for i in range(n):
            iterator = itertools.islice(
                iterable, int(i * chunksize), int((i + 1) * chunksize)
            )
    ...
    def _count_transitions(args):
        ...
        for path, frequency in args["paths"].items():
            mask = toeplitz(
                min(len(path), args["order"]) * [1] + (len(path) - args["order"]) * [0],
                1 * [1] + (len(path) - 1) * [0],
            )
    ...
    def _get_multi_order_transitions(
        self, order, no_of_processes=multiprocessing.cpu_count(), verbose=True
    ):
        n = min(
        ...
        with multiprocessing.Pool(no_of_processes) as p:
            with tqdm(
                ...
                for c in p.imap_unordered(
                    unwrap_self_count_transitions, args, chunksize=1
                ):
                    counter += c
                    pbar.update(1)
    ...
    def _get_multi_order_adjacency_matrix(
        ...
        nodes = list(
            set(n for transition in multi_order_transitions.keys() for n in transition)
        )
    ...
    def _get_multi_order_transition_matrix(
        self, order, no_of_processes=multiprocessing.cpu_count(), A=None, verbose=True
    ):
        if not A:
            A = self._get_multi_order_adjacency_matrix(
                order, no_of_processes=multiprocessing.cpu_count(), verbose=verbose
            )
        ...
        for path, frequency in args["paths"].items():
            ...
            for s, t in zip(multi_order_path, multi_order_path[1:]):
                if s in args["node_id_dict"] and t in args["node_id_dict"]:
                    log_L += (
                        np.log(
                            args["T"][args["node_id_dict"][s], args["node_id_dict"][t]]
                        )
                        * frequency
                    )
    ...
    def _compute_log_likelihood(
        self, order, T, no_of_processes=multiprocessing.cpu_count(), verbose=True
    ):
        n = min(
            int(np.ceil(len(self.paths) / config["MOGen"]["paths_per_chunk"])),
            no_of_processes,
        ...
        with multiprocessing.Pool(no_of_processes) as p:
            with tqdm(
                ...
                for log_likelihood_path in p.imap_unordered(
                    unwrap_self_get_log_likelihood_path, args, chunksize=1
                ):
    ...
    def _compute_AIC(
        ...
        log_L = (
            self._compute_log_likelihood(
                order, T, no_of_processes=no_of_processes, verbose=verbose
            )
            + self.log_L_offset
        )
    ...
    def _compute_order(
        ...
        AIC, log_L, dof = self._compute_AIC(
            order, T, no_of_processes=no_of_processes, verbose=verbose
        )
    ...
    def summary(self, print_summary=True):
        ...
        row["d|dd|fdf (highlight)"] = (
            "\033[1m{:^3d}\033[0m | \033[1m{:^9d} {:^9d}\033[0m |"
        ...
        summary.append(
            row["s|ss|sss"].format("K", "nodes", "edges", "log L", "dof", "AIC")
        )
        ...
        for order in orders:
            ...
            if order == self.optimal_maximum_order:
                summary.append(
                    row["d|dd|fdf (highlight)"].format(order, *[v[-1] for v in data])
                )
            else:
                summary.append(row["d|dd|fdf"].format(order, *[v[-1] for v in data]))
    ...
    def plot(self):
        ...
        highlight = {"color": "#218380", "marker": "o", "markersize": 20, "alpha": 0.3}
    ...
    def _generate_path_chunk(args):
        ...
        for i in range(args["no_of_paths"]):
            ...
            while generated_path[-1][-1] != "+":
                c = np.random.choice(
                    list(args["id_node_dict"].keys()),
                    p=np.ravel(
                        args["mat"][args["node_id_dict"][generated_path[-1]]].todense()
                    ),
                )
    ...
    def pagerank(self, max_order=None):
        ...
        pagerank = pd.DataFrame(
            [v for k, v in c.items()], index=c.keys(), columns=["score"]
    ...
    def mean_first_passage_time(self, max_order=None, recurrence=False):
        ...
        M = MultiOrderMatrix(
            np.zeros(shape=(len(T.node_id_dict), len(T.node_id_dict))), T.node_id_dict
        )
