...
from pathpy3.core.core import PathPyRelation
from pathpy3.core.edge import Edge, EdgeCollection
from pathpy3.core.path import Path, PathCollection
from pathpy3.models.classes import BaseHigherOrderNetwork
from pathpy3.models.network import Network
from pathpy3.algorithms.matrices import transition_matrix
...
class HigherOrderEdge(Edge):
    ...
    def first_order_relations(self):
        """converts edge relations to first order"""
        return PathPyRelation(
            self.v.relations + (self.w.relations[-1],), directed=self.directed
        )
...
class HigherOrderNetwork(BaseHigherOrderNetwork, Network):
    ...
    def __init__(
        self, uid: Optional[str] = None, order: int = 1, **kwargs: Any
    ) -> None:
    ...
    def _(
        self, data: PathCollection, order: Optional[int] = None, subpaths: bool = True
    ) -> None:
    ...
    def possible_relations(self, collection, length: int) -> list:
        ...
        for _ in tqdm(range(length - 1), desc="calculate possible paths"):
            ...
            for _v in possible:
                for _w in edges:
                    if _v[-1] == _w[0]:
                        path = PathPyRelation(_v + (_w[1],), directed=self.directed)
                        new.append(path)
            possible = new
    ...
    def likelihood(self, data: PathCollection, log: bool = False) -> float:
        ...
        for uid, path in data.items():
            ...
            for _v, _w in zip(nodes[:-1], nodes[1:]):
                path_likelihood += np.log(
                    matrix[idx[self.nodes[_w].uid], idx[self.nodes[_v].uid]]
                )
