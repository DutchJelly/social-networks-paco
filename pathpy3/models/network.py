...
# Time-stamp: <Fri 2021-06-04 16:59 juergen>
#
...
from pathpy3.models.classes import BaseNetwork
from pathpy3.core.node import Node, NodeCollection
from pathpy3.core.edge import Edge, EdgeCollection
...
class Network(BaseNetwork):
    ...
    **Edges**
    ...
    def __add__(self, other: Network) -> Network:
        ...
        network = Network(
            directed=self.directed, multiedges=self.multiedges, **self.attributes
        )
    ...
    def _degrees(self, _dict: defaultdict, weight: Weight = None) -> Dict[str, float]:
        ...
        if weight is None:
            ...
            for node in self.nodes:
                _degrees[node.uid] = sum(
                    [self.edges[e].weight(weight) for e in _dict[node]]
                )
        return _degrees
    ...
    def add_edge(
        self,
        *edge: Union[str, tuple, list, Node, Edge],
        uid: Optional[str] = None,
        **kwargs: Any,
    ) -> None:
    ...
    def add_nodes(self, *nodes: Union[str, Node], **kwargs: Any) -> None:
    ...
    def add_edges(
        self, *edges: Union[str, tuple, list, Node, Edge], **kwargs: Any
    ) -> None:
        ...
        if all(isinstance(arg, (str, Node)) for arg in edges) and nodes:
            edges = tuple(
                cast(Union[str, Node], edge) for edge in zip(edges[:-1], edges[1:])
            )
        ...
        for edge in edges:
            self.add_edge(edge, uid=uid, nodes=nodes, update_properties=False, **kwargs)
    ...
    def remove_edge(
        self, *edge: Union[str, tuple, Node, Edge], uid: Optional[str] = None
    ) -> None:
    ...
    def to_multi_layer(
        network: Network, edge_attribute: str, retain_nodes=True
    ) -> dict:
        ...
        network_layers = {}
        for t in edge_types:
            network_layers[t] = Network(
                uid=network.uid + "-" + str(t),
    ...
    def from_temporal_network(
        ...
        **kwargs: Any,
        ...
        network = cls(uid=uid, directed=directed, multiedges=multiedges, **kwargs)
    ...
    def from_paths(cls, path_collection: PathCollection, **kwargs: Any):
        ...
        network = cls(
            uid=uid, directed=path_collection.directed, multiedges=False, **kwargs
        )
        ...
        for p in path_collection:
            for i in range(1, len(p) + 1):
                network.add_edge(
                    p.relations[i - 1],
                    p.relations[i],
                    count=path_collection.counter[p.uid],
                )
