...
from pathpy3.models.classes import BaseMultiOrderModel
...
class MultiOrderModel(BaseMultiOrderModel):
    ...
    def __init__(
        self, uid: Optional[str] = None, max_order: int = 1, **kwargs: Any
    ) -> None:
    ...
    def fit(
        ...
        for order in list(range(self._current_max_order() + 1, self.max_order + 1)):
            ...
            _hon = HigherOrderNetwork.from_paths(data, order=order, subpaths=True)
    ...
    def predict(self, data: Optional[PathCollection] = None, threshold=0.01):
        ...
        for order in range(2, max_considerd_order + 1):
            ...
            accept, _ = self.likelihood_ratio_test(
                data, null=order - 1, order=order, threshold=threshold
            )
        ...
        LOG.debug(
            "end estimate optiomal order: %s seconds", (end - start).total_seconds()
        )
    ...
    def likelihood_ratio_test(self, data, null=0, order=1, threshold=0.01):
        ...
        LOG.debug(
            "end likelihood ratio test: %s seconds", (end - start).total_seconds()
        )
    ...
    def likelihood(self, data, order=1, log=True):
        ...
        for order in range(0, max_order + 1):
            likelihood += self.layer_likelihood(
                data=data, order=order, longer_paths=(order == max_order), log=True
            )
    ...
    def layer_likelihood(
        self, data, order=1, longer_paths=True, log=True, min_length=None
    ):
        """Layer Likelihood"""
    ...
    def path_likelihood(self, path, frequency, order=1, log=True):
        ...
        if order == 0:
            for _n in edges:
                likelihood += np.log(hon.nodes.counter(hon.nodes[_n].uid)) * frequency
        else:
            for _v, _w in edges:
                ...
                likelihood += (
                    np.log(
                        self.layers[order]["T"][
                            n[hon.nodes[_w].uid], n[hon.nodes[_v].uid]
                        ]
                    )
                    * frequency
                )
        ...
        for _order, _e in transitions.items():
            ...
            if _order == 0:
                likelihood += np.log(_hon.nodes.counter[_hon.nodes[_e].uid]) * frequency
            else:
                # get a list of nodes for the matrix indices
                n = _hon.nodes.index
                _v = _e[0]
                ...
                likelihood += (
                    np.log(
                        self.layers[_order]["T"][
                            n[_hon.nodes[_w].uid], n[_hon.nodes[_v].uid]
                        ]
                    )
                    * frequency
                )
    ...
    def summary(self):
        ...
        summary.append(
            row["s|ss|ss"].format("order", "nodes", "edges", "paths", "ngrams")
        )
        ...
        for order, models in self.layers.items():
            ...
            summary.append(row["d|dd|dd"].format(order, *[v[-1] for v in data]))
