...
# =============================================================================
import numpy as np
from pathpy3 import config, logger, tqdm
...
class Layout(object):
    ...
    def generate_layout(self):
        ...
        elif self.layout_type in names_circular or (
            self.layout_type == "lattice" and self.dimension == 1
        ):
            self.layout = self.circular()
        elif self.layout_type in names_grid or (
            self.layout_type == "lattice" and self.dimension == 2
        ):
        ...
        **Keyword arguments used for the layout:**
        ...
        if self.positions is not None:
            ...
            if _size == 0:
                _size = 1
            np.random.seed(self.seed)
            self.layout = np.random.rand(len(self.nodes), self.dimension) * _size
    ...
    def _fruchterman_reingold(self):
        ...
        try:
            ...
            log.error(
                "Fruchterman-Reingold algorithm needs an adjacency " "matrix as input"
            )
        ...
        t = (
            max(
                max(layout.T[0]) - min(layout.T[0]), max(layout.T[1]) - min(layout.T[1])
            )
            * 0.1
        )
        ...
        for iteration in tqdm(
            range(self.iterations), desc="Calculating Fruchterman-Reingold layout"
        ):
    ...
    def _sparse_fruchterman_reingold(self):
        ...
        try:
            ...
            log.error(
                "The sparse Fruchterman-Reingold algorithm needs the "
