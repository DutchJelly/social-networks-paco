...
from pathpy3.core.api import PathCollection, Path
...
class SubPathCollection(PathCollection):
    ...
    def __call__(
        ...
        if len(self) == 0 or recalculate:
            self.calculate(
                min_length=min_length, max_length=max_length, include_path=include_path
            )
        return self
    ...
    def summary(self) -> str:
        ...
        summary.append(
            row["s|sss|ss"].format("length", "obs", "pos", "tot", "obs", "pos")
        )
        ...
        for order in range(max_order + 1):
            ...
            data[3].append(np.count_nonzero(list(self.observed[order].values())))
            data[4].append(np.count_nonzero(list(self.possible[order].values())))

            summary.append(row["f|fff|ff"].format(order, *[v[-1] for v in data]))
