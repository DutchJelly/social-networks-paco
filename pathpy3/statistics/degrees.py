...
from pathpy3.models.classes import BaseModel
...
def degree_distribution(
    degrees: Union[Network, Iterable], weight: Weight = None
) -> Dict[float, float]:
...
def mean_neighbor_degree(
    network, weight: Weight = None, exclude_neighbor=False
) -> float:
    """Calculates the mean (weighted degree of a network)"""
    neighbor_degrees = []
    for v in network.nodes.uids:
...
def degree_raw_moment(network: Network, k: int = 1, weight: Weight = None) -> float:
...
def degree_central_moment(network: Network, k: int = 1, weight: Weight = None) -> float:
...
def degree_generating_function(
    degrees: Union[Network, Iterable],
...
def molloy_reed_fraction(network: Network, weight: Weight = False) -> float:
    ...
    """
    _mrf = degree_raw_moment(network, k=2, weight=weight) / degree_raw_moment(
        network, k=1, weight=weight
    )
    return _mrf


def degree_assortativity(
    network: Network, mode: str = "total", weight: Weight = None
) -> float:
