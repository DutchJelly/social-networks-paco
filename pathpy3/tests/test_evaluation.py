...
def test_train_test_split_network(net):
    ...
    """
    train, test = pp.algorithms.evaluation.train_test_split(
        net, split="node", test_size=0.25
    )
...
def test_train_test_split_temporalnetwork(temp_net):
    ...
    """
    train, test = pp.algorithms.evaluation.train_test_split(
        temp_net, split="time", test_size=0.5
    )
    ...
    train, test = pp.algorithms.evaluation.train_test_split(
        temp_net, split="interactions", test_size=0.5
    )
    assert train.number_of_edges() == 2
    assert test.number_of_edges() == 2
    assert train.number_of_edges() == 2
