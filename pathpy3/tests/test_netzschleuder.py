...
def get_metric(network: pp.Network, metric: str):
    switcher = {
        ...
        "global_clustering_coefficient": pp.statistics.avg_clustering_coefficient(
            network
        ),
        "diameter": pp.algorithms.diameter(network),
        "edge_reciprocity": pp.statistics.edge_reciprocity(network),
        "largest_component_fraction": pp.algorithms.largest_component_size(network)
...
def test_netzschleuder_consistency():
    for data, net, multi in datasets:
        ...
        network = pp.io.graphtool.read_netzschleuder_network(
            data, net, multiedges=multi
        )
        ...
        for metric in consistency_checks:
            ...
            if val is None or netzschleuder_analysis[metric] != pytest.approx(val):
                print(
                    "Consistency check failed for network {0} and metric {1}: {2} != {3}".format(
                        data, metric, netzschleuder_analysis[metric], val
                    )
                )
