...
def to_state_file(paths: PathCollection, file: str, max_memory: int = 1) -> None:
    ...
    *Links
    ...
    with open(file, mode="w") as f:
        ...
        nodes_to_index = OrderedDict(
            {i[0]: i[1] for i in zip(sorted(nodes), range(1, n + 1))}
        )
        for v in nodes:
        ...
        for index, item in states_by_index.items():
            state_file.append(
                '{0} {1} "{2}"'.format(index, nodes_to_index[item[0]], item[1])
            )
        ...
        for l in links:
            state_file.append(
                "{0} {1} {2}".format(
                    states_to_index[l[0]], states_to_index[l[1]], links[l]
                )
            )
        f.write("\n".join(state_file))
