...
import json
from pathpy3.utils.errors import FileFormatError, NetworkError
import pickle
import struct
...
from pathpy3.io.pandas import to_network, to_temporal_network
from pathpy3.models.network import Network
from pathpy3.models.temporal_network import TemporalNetwork
from pathpy3 import FileFormatError, NetworkError, MissingModuleError
...
def _parse_property_value(
    data: bytes, ptr: int, type_index: int, endianness: str
    ...
    if type_index == 0:
        ...
        LOG.warning(
            "pathpy does not support properties with type long double. Properties have been dropped."
        )
        return (None, 16)
    elif type_index == 6:
        ...
        for i in range(num_values):
            vals.append(
                struct.unpack(endianness + "h", data[ptr + offset : ptr + offset + 2])[
                    0
                ]
            )
            offset += 4
        ...
        for i in range(num_values):
            vals.append(
                struct.unpack(endianness + "i", data[ptr + offset : ptr + offset + 4])[
                    0
                ]
            )
            offset += 4
        ...
        for i in range(num_values):
            vals.append(
                struct.unpack(endianness + "Q", data[ptr + offset : ptr + offset + 8])[
                    0
                ]
            )
            offset += 8
        ...
        for i in range(num_values):
            vals.append(
                struct.unpack(endianness + "d", data[ptr + offset : ptr + offset + 8])[
                    0
                ]
            )
            offset += 8
        ...
        LOG.warning(
            "pathpy does not support properties with type vector<long double>. Properties have been dropped."
        )
        ...
        for i in range(num_strings):
            str_len = struct.unpack(
                endianness + "Q", data[ptr + offset : ptr + offset + 8]
            )[0]
            offset += 8
...
def parse_graphtool_format(
    data: bytes, ignore_temporal: bool = False, multiedges: bool = False
) -> Union[Network, TemporalNetwork]:
    """
    ...
    for v in range(n_nodes):
        ...
        num_neighbors = struct.unpack(graphtool_endianness + "Q", data[ptr : ptr + 8])[
            0
        ]
        ptr += 8
    ...
    network_data = pd.DataFrame.from_dict(
        network_dict, orient="index", columns=["v", "w"]
    )
    ...
    for i in range(property_maps):
        ...
        property_type = struct.unpack(graphtool_endianness + "B", data[ptr : ptr + 1])[
            0
        ]
        ptr += 1
        ...
        if key_type == 0:  # network property
            ...
            for v in range(n_nodes):
                res = _parse_property_value(
                    data, ptr, property_type, graphtool_endianness
                )
    ...
    if "time" in edge_attribute_names and not ignore_temporal:
        ...
        n = to_network(
            network_data, directed=directed, multiedges=multiedges, **network_attributes
        )
...
def read_graphtool(
    file: str, ignore_temporal: bool = False, multiedges: bool = False
) -> Optional[Union[Network, TemporalNetwork]]:
    """
    ...
    with open(file, "rb") as f:
        if ".zst" in file:
            try:
                ...
                data = f.read()
                return parse_graphtool_format(
                    dctx.decompress(data, max_output_size=len(data))
                )
...
def list_netzschleuder_records(
    base_url: str = "https://networks.skewed.de", **kwargs
) -> Union[list, dict]:
...
def read_netzschleuder_record(
    name: str, base_url: str = "https://networks.skewed.de"
) -> dict:
    """
...
def read_netzschleuder_network(
    ...
    try:
        ...
        try:
            ...
            msg = "Could not connect to netzschleuder repository at {0}".format(
                base_url
            )
            LOG.error(msg)
            raise NetworkError(msg)
        ...
        n = parse_graphtool_format(
            bytes(decompressed), ignore_temporal=ignore_temporal, multiedges=multiedges
        )
        if n:
