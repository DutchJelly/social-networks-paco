...
def _detect_nodes(edges, node_radius: Tuple[int, int], num_nodes: int, n):
    ...
    accums, cx, cy, radii = hough_circle_peaks(
        hough_res, hough_radii, total_num_peaks=num_nodes
    )
    ...
    for center_y, center_x, radius in zip(cy, cx, radii):
        ...
        n.add_node(
            str(i), x=center_x, y=center_y, r=radius, coordinates=(center_x, -center_y)
        )
        i += 1
...
def _detect_edges(
    ...
    lines = probabilistic_hough_line(
        edges, threshold=threshold, line_length=min_edge_length, line_gap=max_edge_gap
    )
...
def from_image(
    ...
    sigma=0.25,
    dilation=2,
    node_radius: tuple = (10, 15),
    num_nodes: int = 50,
    threshold=20,
    min_edge_length: int = 20,
    max_edge_gap: int = 5,
    intersect_tolerance_factor: int = 1.1,
) -> Network:
    """ """
    ...
    _detect_edges(
        edges, min_edge_length, max_edge_gap, threshold, intersect_tolerance_factor, n
    )
