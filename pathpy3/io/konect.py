...
# =============================================================================
from pathpy3.utils.errors import NetworkError
import tarfile
import io
...
from pathpy3.models.api import TemporalNetwork
from pathpy3 import FileFormatError, NetworkError

# create logger
LOG = logger(__name__)


def read_tsv_network(
    file: str, ignore_temporal: bool = False
                ...
                if bytedata:
                    with io.TextIOWrapper(bytedata) as buffer:
                        ...
                        network_data.columns = [
                            tsv_columns[i] for i in range(len(network_data.columns))
                        ]
                        duplicates = len(
                            network_data[
                                network_data.duplicated(["v", "w"], keep=False)
                            ]
                        )
                        if duplicates > 0:
                            ...
                            multiedges = True
                        LOG.info(
                            "Detected columns: {0}".format(
                                str([c for c in network_data.columns])
                            )
                        )
                else:
                    msg = "Could not extract tar file {0}".format(f)
    ...
    if "timeiso" in attributes:
        try:
            ...
            LOG.warning(
                "KONECT data contains invalid timeiso: {}".format(attributes["timeiso"])
            )
    if "time" in network_data.columns and not ignore_temporal:
        network_data.rename(columns={"time": "timestamp"}, inplace=True)
        return to_temporal_network(
            network_data, directed=directed, multiedges=multiedges, **attributes
        )
    else:
        return to_network(
            network_data, directed=directed, multiedges=multiedges, **attributes
...
def read_konect_name(
    name, ignore_temporal: bool = False, base_url="http://konect.cc/files/download.tsv."
