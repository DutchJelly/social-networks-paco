...
from pathpy3 import config, logger
from pathpy3.core.core import PathPyRelation
from pathpy3.core.api import Node
from pathpy3.core.api import Edge
    ...
    **kwargs: Any
    ...
    for row in df.to_dict(orient="records"):
        ...
        if (_v, _w) in edge_set and not multiedges:
            ...
            edges.append(Edge(nodes[_v], nodes[_w], uid=uid, directed=directed, **row))
            edge_set.add((_v, _w))
            if not directed:
                edge_set.add((_w, _v))
        else:
...
def to_temporal_network(
    ...
    **kwargs: Any
    ...
    from pathpy3.models.temporal_network import (
        TemporalNetwork,
        TemporalEdge,
        TemporalNode,
    )
    ...
    for key, name in _key_words.items():
        df = _check_column_name(df, name, config["temporal"][key + "_synonyms"])
    ...
    for row in df.to_dict(orient="records"):
        ...
        uid = row.pop("uid", None)
        net.add_edge(nodes[_v], nodes[_w], uid=uid, update_properties=False, **row)
...
def from_network(
    network: Network,
    include_edge_uid: Optional[bool] = False,
...
def to_dataframe(
    ...
    if isinstance(network, TemporalNetwork):
        frame = from_temporal_network(
            network, include_edge_uid=include_edge_uid, export_indices=export_indices
        )
